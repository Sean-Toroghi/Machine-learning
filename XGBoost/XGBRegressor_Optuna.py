# Three tier hyperparameter tuning for XGBoost Regressor model
# Each tier consists of a range of hyperparameters and they are categorized by their impact on the model performance from high to granular (fine-tuning) level.
...
# -----------------------------------------------------------------------------------
def objective_xgb(trial, X_train, y_train, X_valid, 
                  y_valid, weights_train, 
                  weights_valid, cat_cols):
    ...
    params = {
        ...
        'n_estimators': trial.suggest_int('n_estimators', 1750, 2050),
...
def objective_xgb(trial, X_train, y_train, 
                  X_valid, y_valid, weights_train, 
                  xweights_valid, cat_cols):
    # add early stopping rounds
    es = callback.EarlyStopping(rounds=50,
                                        #min_delta=1e-3,
                                        #save_best=True,
...
def objective_xgb(trial, X_train, y_train, X_valid, y_valid, 
                  weights_train, weights_valid, cat_cols):
    # add early stopping rounds
    es = callback.EarlyStopping(rounds=50,
                                        #min_delta=1e-3,
                                        #save_best=True,
